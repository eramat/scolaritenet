...
function liste_lettres($lettre) {
    ...
    // Recherche de l'existence de personnes dont le nom commence par certaines lettres de l'alphabet
    $req = "SELECT DISTINCT(ORD(UPPER(SUBSTRING(login FROM 2 FOR 1)))) AS lettre
    ...
    for ($l=ord("A"); $l<=ord("Z"); $l++) {
        if ($lettre == $l) { // Lettre sélectionnée
            ...
            echo "<a href=\"javascript:choix_lettre('us', ",$l,");\">[",chr($l),"]</a> ";
        } else {
            echo "[",chr($l),"] ";
        }
    }
...
if (isset($_POST["desactiver"]) && $_POST["desactiver"]) {
    mysql_query("update ".$prefix_tables."users set actif=0 where id_user=".$_POST["desactiver"]);
}

if ($lettre) {
    $res = $DB->Execute("SELECT DISTINCT(u.id_user), u.login, u.actif
                         FROM ".$prefix_tables."users u, ".$prefix_tables."user_est_de_type ut
