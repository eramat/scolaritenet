...
function charge_donnees($id) {
    ...
    $request = "SELECT p.id_periode, p.numero, p.date_debut, p.date_fin
                FROM ".$prefix_tables."calendrier, ".$prefix_tables."calendrier_travail c, ".$prefix_tables."periode_travail p
    ...
    $request = "SELECT f.id_periode, f.nom, f.date_debut, f.date_fin
                FROM ".$prefix_tables."calendrier, ".$prefix_tables."calendrier_ferie c, ".$prefix_tables."periode_ferie f
    ...
    while ($row = $res->FetchRow())
    {
        $donnees["ferie"][$i] = array("id_periode" => $row[0],"nom" => $row[1],
                                      "date_debut" => $row[2],"date_fin" => $row[3]);
        $i++;
    }
    $res->Close();
...
function ajout_periode_travail($id) {
    ...
    $request_data = array($i, $date_debut, $date_fin);
...
function ajout_periode_ferie($id) {
    ...
    $request = "INSERT INTO ".$prefix_tables."periode_ferie (nom, date_debut, date_fin) VALUES (?, ?, ?)";
    $request_data = array("", $date_debut, $date_fin);
    $DB->Execute($request, $request_data);
    $id_periode = $DB->Insert_ID();
    $request = "INSERT ".$prefix_tables."calendrier_ferie (id_calendrier, id_periode) VALUES (?, ?)";
    $request_data = array($id, $id_periode);
    $DB->Execute($request, $request_data);
}
...
function verifie_existence_calendrier($id) {
    global $prefix_tables, $DB;
    return $DB->GetOne("select count(*) from ".$prefix_tables."calendrier where id=".$id);
}
