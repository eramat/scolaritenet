...
$req .= " FROM ".$prefix_tables."module m, ".$prefix_tables.
  "module_suivi_diplome msd";
$req .= " WHERE (msd.id_diplome=? and m.id=msd.id_module)";
$req .= " ORDER BY m.num_periode, m.id asc)";
$req .= " UNION ";
$req .= "(SELECT DISTINCT(m.id), m.nom, m.num_periode";
$req.= " FROM ".$prefix_tables."module_suivi_option mso, ".
  $prefix_tables."option o, ".$prefix_tables."module m, ".
  $prefix_tables."module_suivi_diplome msd";
$req .=	" WHERE (o.id_diplome=? and mso.id_option = o.id and
          m.id = mso.id_module) ";
$req .= " ORDER BY m.num_periode, m.id asc)";
...
if ($res->RecordCount() > 0) {
  ...
  while ($row = $res->FetchRow()) { // pour chaque module
	...
	$req5 = "select (e.nom || ' ' || e.prenom) as nom_complet_enseignant";
