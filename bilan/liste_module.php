...
} elseif (isset($_GET["id_diplome"]) &&
	  est_directeur_de_departement($_SESSION["usertype"])) {
...
if (est_etudiant($_SESSION["usertype"])) {
  $request = "SELECT d.id_diplome, d.sigle_complet
              FROM ".$prefix_tables."diplome d, ".$prefix_tables.
    "inscrit_diplome id
...
if ($request) {
  ...
  if ($result->RecordCount() > 1) { // Il y a plusieurs diplômes -> il faut choisir !
    ...
    while ($a_record = $result->FetchRow()) {
      echo "<option value=\"".$a_record[0]."\"",
	(($id_diplome == $a_record[0]) ? " selected" : ""),">",$a_record[1],
	"</option>";
    }
    echo "</select>";
    echo "</p>";
