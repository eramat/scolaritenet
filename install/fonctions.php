...
function affiche_header()
{
	echo '<html>
			...
			<div id="info">Bienvenue dans le module d\'installation de ScolariteNet v0.9</div>';
}
...
{
	if ( empty ($_POST['dbtype']) ) ajout_erreur('Veuillez sp&eacute;cifier le type de la base.');
	if ( empty ($_POST['dbhost']) ) ajout_erreur('Veuillez indiquer l\'hôte de la base.');
	if ( empty ($_POST['dbname']) ) ajout_erreur('Veuillez indiquer le nom de la base.');
	if ( empty ($_POST['dbuser']) ) ajout_erreur('Veuillez indiquer un nom d\'utilisateur pour se connecter à la base.');
	if ( empty ($_POST['racine_site']) ) ajout_erreur('Veuillez indiquer le r&eacute;pertoire racine de scolaritenet.');
	if ( !empty($_SESSION['erreur']) ) return false;
...
function installer()
{
  ...
  if (!$req = @file_get_contents('./types/'.$GLOBALS['$install_type'].'.sql')) {
  ...
  $file = fopen('../includes/configuration.php', 'w');
  if (!$file) {
    error('Impossible de cr&eacute;er le fichier ../includes/configuration.php');
  }
  if (!@fwrite($file, $conf)) {
    error('Impossible d\'&eacute;crire dans le fichier ../includes/configuration.php');
  }
  fclose($file);
