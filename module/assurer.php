...
function types_seances() {
    ...
    $res = $DB->Execute("select id, libelle from ".$prefix_tables."type_sceance order by id asc");
...
function charger_types_associes_au_groupe($id_groupe) {
    ...
    if ($id_groupe) {
        ...
        $res = $DB->Execute("select ts.id
                            from ".$prefix_tables."type_sceance ts
...
function liste_enseignants($id_enseignant, $tab_enseignants, $id_module, $kolor, $val_retour) {
    global $prefix_tables, $DB, $line_color, $tab_action, $types_seances;
    
    // Récupération des enseignants associés à ce module pour empêcher l'utilisateur de faire un choix incorrect
    $tab_disabled = array();
