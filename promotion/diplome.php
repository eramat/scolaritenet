...
// Définition des valeurs des types enseignant, président du jury et
...
function affiche_liste_diplomes($id_niveau, $annee, $id_domaine, $val_retour) {
  ...
  if ($res->RecordCount()) {
    ...
    while ($row = $res->FetchRow()) {
      echo "<a href=\"javascript:choix_diplome(",$row[0],");\">",
	$row[1], "</a><br>";
    }
  } else {
  ...
  echo "</tr><tr>\n";
  echo "<td colspan=\"2\" align=\"center\"><br><input type=\"button\"
          value=\"Nouveau dipl&ocirc;me\" onClick=\"choix.value=",$val_retour,
    "; submit();\"></td>";
}
...
function affiche_details_type($type, $selected) {
  ...
  while ($row = $res->FetchRow()) {
    echo "<option value=\"",$row[0],"\"",
      (($row[0]==$selected) ? " selected" : ""),">",$row[1],"</option>\n";
  }
  $res->Close();
}
...
function charge_donnees_diplome($id_diplome) {
  ...
  $row = $DB->GetRow("SELECT id_niveau, annee, id_domaine, id_mention,
                             id_specialite,
                             intitule_parcours, sigle, sigle_complet,
                             id_directeur_etudes,
                             id_president_jury, id_pole, id_departement,
                             id_calendrier
                      FROM ".$prefix_tables."diplome
                      WHERE id_diplome = ".$id_diplome);
  $donnees = array("niveau" => $row[0], "annee" => $row[1],
		   "domaine" => $row[2],
		   "mention" => $row[3], "specialite" => $row[4],
		   "parcours" => $row[5], "sigle" => $row[6],
		   "sigle_complet" => $row[7],
		   "directeur_etudes" => $row[8], "president_jury" => $row[9],
		   "pole" => $row[10], "departement" => $row[11],
		   "calendrier" => $row[12]);
  return $donnees;
}
...
function affiche_formulaire($id_diplome, $val_retour_verif, $val_retour_liste) {
  ...
  if (isset($_POST["sigle"])) {
    ...
    $donnees = array("niveau" => ((isset($_POST["niveau"])) ?
				  $_POST["niveau"] : 0),
		     "annee" => ((isset($_POST["annee"])) ?
				 $_POST["annee"] : ""),
		     "domaine" => ((isset($_POST["domaine"])) ?
				   $_POST["domaine"] : 0),
		     "mention" => 0, "specialite" => 0, "parcours" => "",
		     "sigle" => "", "sigle_complet" => "",
		     "directeur_etudes" => 0, "president_jury" => 0,
		     "pole" => 0, "departement" => 0, "calendrier" => 0);
  }
  ...
  echo "<tr><td>Parcours</td><td><input type=\"text\" name=\"parcours\" size=\"20\" maxlength=\"32\" value=\"",$donnees["parcours"],"\"> <i>(facultatif)</i></td></tr>\n";
  // Sigle
...
function affecter_droits($id_diplome, $id_type, $nom_type, $id_enseignant) {
  ...
  // En cas de mise à jour, on regarde si on doit supprimer les droits
  // du précédent enseignant
  if ($id_diplome) {
    ...
    if ($nbRows == 1) {
      $DB->Execute("DELETE FROM ".$prefix_tables."user_est_de_type
                    WHERE id_type=".$id_type." AND id=".
		   $id_precedent_enseignant);
    }
  }
  ...
  $id_user = $DB->GetOne("SELECT id_user
                          ...
                          WHERE id_type=".$id_type_enseignant." AND id=".
			 $id_enseignant);
...
function ajoute_donnees($id_diplome) {
  global $prefix_tables, $DB, $id_type_directeur_des_etudes,
    $id_type_president_du_jury;

  affecter_droits($id_diplome, $id_type_directeur_des_etudes,
		  "directeur_etudes", $_POST["directeur_etudes"]);
  affecter_droits($id_diplome, $id_type_president_du_jury,
		  "president_jury", $_POST["president_jury"]);

  if ($id_diplome) {
    $DB->Execute("UPDATE ".$prefix_tables."diplome
                  SET id_niveau=?, annee=?, id_domaine=?, id_mention=?,
                      id_specialite=?,
                      intitule_parcours=?, sigle=?, sigle_complet=?,
                      id_directeur_etudes=?,
                      id_president_jury=?, id_pole=?, id_departement=?,
                      id_calendrier=?
                  WHERE id_diplome=?",
		 array($_POST["niveau"], $_POST["annee"], $_POST["domaine"],
		       $_POST["mention"],
		       $_POST["specialite"], $_POST["parcours"],
		       strtoupper($_POST["sigle"]),
		       $_POST["sigle_complet"], $_POST["directeur_etudes"],
		       $_POST["president_jury"],
		       $_POST["pole"], $_POST["departement"],
		       $_POST["calendrier"], $id_diplome));
  } else {
    $_POST["diplome"] = $DB->GenID($prefix_tables."diplome_id_seq");
    $req = "INSERT INTO ".$prefix_tables."diplome
            (id_diplome, id_niveau, annee, id_domaine, id_mention,
             id_specialite, intitule_parcours, sigle, sigle_complet,
             id_directeur_etudes, id_president_jury, id_pole, id_departement,
             id_calendrier)
            VALUES (".$_POST["diplome"].", ".$_POST["niveau"].", ".$_POST["annee"].", ".$_POST["domaine"].", ".$_POST["mention"].", ".$_POST["specialite"].", '".$_POST["parcours"]."', '".strtoupper($_POST["sigle"])."', '".$_POST["sigle_complet"]."', ".$_POST["directeur_etudes"].", ".$_POST["president_jury"].", ".$_POST["pole"].", ".$_POST["departement"].", ".$_POST["calendrier"].")";
      $DB->Execute($req);
    }
}
...
if (isset($_POST["choix"])) {
  ...
  case $tab_ret["domaine"] : // Choix du domaine
	...
	if ($_POST["domaine"]) {
	  ...
	  affiche_liste_diplomes($_POST["niveau"], $_POST["annee"],
				 $_POST["domaine"], $tab_ret["diplome"]);
	}
      }
    }
    break;
  ...
  case $tab_ret["diplome"] :
    affiche_formulaire($_POST["diplome"], $tab_ret["verif"],
		       $tab_ret["domaine"]);
    break;
