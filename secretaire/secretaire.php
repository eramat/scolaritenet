...
function afficher_liste() {
    ...
    $request = "SELECT id_secretaire, concat(nom,' ',prenom) AS nom_complet 
	...
	if ($res->RecordCount()) {
		...
		while ($row = $res->FetchRow()) {
            echo "<a href=\"index.php?page=secretaire&id=",$row[0],"\">", $row[1], "</a><br />\n";
        }
        echo "</p>\n";
    } else {
...
function creer_pseudo($nom, $prenom, $id) {
	...
	$request = "INSERT INTO ".$prefix_tables."users (login, password, actif) 
				VALUES (?, ?, ?)";
	$param_array = array($pseudo, md5(""), 0);
	$DB->Execute($request, $param_array);
	$id_user = $DB->Insert_ID();
    // Ajout des données
	$request = "INSERT INTO ".$prefix_tables."user_est_de_type (id_user, id_type, id) 
				VALUES (?, ?, ?)";
...
function ajouter_secretaire($donnees) {
	...
	$request = "INSERT INTO ".$prefix_tables."secretaire (nom, prenom, tel, fax, email, id_pole) 
				VALUES (?, ?, ?, ?, ?, ?)";
...
function supprimer_secretaire($id_secretaire) {
    ...
    $request = "DELETE FROM ".$prefix_tables."secretaire_occupe_diplome WHERE id_secretaire=?";
	$DB->Execute($request, array($id_secretaire));
}
