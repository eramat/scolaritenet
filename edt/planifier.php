...
if (($id_diplome>0 OR $id_option>0) AND $id_periode>0 AND $G_nb_modules>0 AND $s_semaine>0) {
    ...
    for ($i=0; $i<$G_nb_modules; $i++) {
        if ($id_groupe <= 0) {
            $request = "SELECT ts.libelle, mr.nombre_heures, ts.id
                        FROM ".$prefix_tables."type_sceance ts, ".$prefix_tables."module_reparti mr
        ...
        while ($row = $result->FetchRow()) {
            $a_record = array("id" => $row[2], "libelle" => $row[0], "nombre_heures" => $row[1]);
        ...
        for ($j=1; $j<=6; $j++) {
            ...
            if (!isset($creneau[($i-8)*2][$j]) or $creneau[($i-8)*2][$j] == 4) {
                ...
                if ($result->RecordCount()) {
                    $row = $result->FetchRow();
                    $a_record = array("T0" => $row[0], "T1" => $row[1], "T2" => $row[2], "T3" => $row[3],
